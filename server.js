...
const emptyData = { students: [], staff: [], results: [], parents: [], users: [], feeStructures: [], timetableSlots: [], communications: [], operations: [] };
...
function writeData(data) { fs.writeFileSync(DATA_FILE, JSON.stringify({ ...emptyData, ...data }, null, 2)); }
function allUsers() {
  const data = readData();
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/api/health') return send(res, 200, { ok: true, school: 'Mahanaim Academy', storage: 'JSON database', authentication: 'session cookie' });
  if (req.url === '/api/data' && req.method === 'GET') return send(res, 200, readData());
  if (req.url === '/api/data' && req.method === 'PUT') { if(!currentUser(req)) return send(res,401,{ok:false,error:'Sign-in required'}); try { const body=await readBody(req); writeData(body); return send(res, 200, { ok:true, savedAt:new Date().toISOString() }); } catch { return send(res,400,{ok:false,error:'Invalid JSON payload'}); } }
  if (req.url === '/api/auth/login' && req.method === 'POST') { try { const body=await readBody(req); const identity=String(body.identity||'').toLowerCase(); const password=String(body.password||''); const user=allUsers().find(u=>(u.email||'').toLowerCase()===identity || (u.name||'').toLowerCase()===identity); if(!user || hashPassword(password)!==user.passwordHash) return send(res,401,{ok:false,error:'Invalid sign-in details'}); const token=crypto.randomBytes(24).toString('hex'); sessions.set(token,publicUser(user)); return send(res,200,{ok:true,user:publicUser(user) },'application/json',{'Set-Cookie':`mahanaim_session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=28800`}); } catch { return send(res,400,{ok:false,error:'Invalid sign-in request'}); } }
  if (req.url === '/api/auth/users' && (req.method === 'GET' || req.method === 'POST')) { const actor=currentUser(req); if(!actor || actor.role!=='Administrator') return send(res,403,{ok:false,error:'Administrator access required'}); if(req.method==='GET') return send(res,200,{ok:true,users:allUsers().map(publicUser)}); try { const body=await readBody(req); const name=String(body.name||'').trim(),email=String(body.email||'').trim().toLowerCase(),role=String(body.role||'Teacher').trim(),password=String(body.password||''); if(!name||!email||password.length<8) return send(res,400,{ok:false,error:'Name, email and a password of at least 8 characters are required'}); const data=readData(); const users=Array.isArray(data.users)?data.users:[]; if(users.some(u=>(u.email||'').toLowerCase()===email) || email==='admin@mahanaim.edu.gh') return send(res,409,{ok:false,error:'A user with that email already exists'}); const user={id:`USR-${String(100+users.length+1).padStart(3,'0')}`,name,email,role,status:'Active',passwordHash:hashPassword(password)}; users.push(user); data.users=users; writeData(data); return send(res,201,{ok:true,user:publicUser(user)}); } catch { return send(res,400,{ok:false,error:'Invalid user payload'}); } }
...
if (!fs.existsSync(DATA_FILE)) writeData(emptyData);
server.listen(PORT, '0.0.0.0', () => console.log(`Mahanaim Academy server listening on ${PORT}`));
