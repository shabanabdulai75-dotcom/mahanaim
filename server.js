...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.url === '/api/auth/users' && (req.method === 'GET' || req.method === 'POST')) { const actor=currentUser(req); if(!actor || actor.role!=='Administrator') return send(res,403,{ok:false,error:'Administrator access required'}); if(req.method==='GET') return send(res,200,{ok:true,users:(await allUsers()).map(publicUser)}); const body=await readBody(req); const name=String(body.name||'').trim(),email=String(body.email||'').trim().toLowerCase(),role=String(body.role||'Teacher').trim(),password=String(body.password||''); if(!name||!email||password.length<8) return send(res,400,{ok:false,error:'Name, email and a password of at least 8 characters are required'}); const data=await readData(); const users=Array.isArray(data.users)?data.users:[]; if(users.some(u=>(u.email||'').toLowerCase()===email) || email==='admin@mahanaim.edu.gh') return send(res,409,{ok:false,error:'A user with that email already exists'}); const user={id:`USR-${String(100+users.length+1).padStart(3,'0')}`,name,email,role,status:'Active',passwordHash:hashPassword(password)}; users.push(user); data.users=users; await writeData(data); return send(res,201,{ok:true,user:publicUser(user)}); }
